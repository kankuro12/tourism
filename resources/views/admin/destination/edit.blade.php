...
@section('s-title')
    <a href="{{ route('admin.destination.index',['type'=>$type->id]) }}">Destinations - {{$type->name}}</a> / {{$destination->name}} / Edit
@endsection

@section('content')
    <form action="{{ route('admin.destination.edit',['destination'=>$destination->id]) }}" enctype="multipart/form-data" method="post">
        @csrf
        <div class="bg-white shadow mb-3">
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name"  class="form-control " required value="{{$destination->name}}">
                </div>
                <div class="form-group">
                    <label for="image">Image (1980 X 1000)</label>
                    <input type="file" name="image" id="image" accept="image/*" class="form-control image" data-default-file="{{asset($destination->image)}}" >
                </div>

                <div class="form-group">
                    <label for="desc">Full Description</label>
                    <textarea name="desc" id="desc" class="form-control">{{$destination->desc}}</textarea>
                </div>
                ...
                <div>
                    <div class="shadow mb-3">
                        <h4 class="d-flex justify-content-between align-items-center px-3 py-1">
                            ...
                            <input type="text" id="map" name="map" class="form-control w-25" placeholder="Search Place" required value="{{$destination->map}}">
                        </h4>
                        <hr class="m-0">
